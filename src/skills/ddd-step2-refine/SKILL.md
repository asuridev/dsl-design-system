...
- Los campos de cada entry son `aggregate`, `param` y `error` (no `field` ni `notFoundError`).
...
- **`errorCode` obligatorio en toda domainRule excepto `sideEffect`**:
  ...
  - Cada `errorCode` debe existir en `errors[]` (validación cruzada con E5).
...
- **`bulk` block**: `{ itemType, maxItems ≥1, onItemError: continue|abort }`.
  - `itemType` debe ser un VO o projection declarado.
