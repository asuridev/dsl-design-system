...
- **Un solo calificador por método `count`.** El generador solo reconoce calificadores simples: un literal exacto del enum de status (`Active`, `Draft`, `Confirmed`…), su forma negada (`Non{Literal}` → `status <> 'LITERAL'`), o `NonDeleted`/`Deleted`. **Los calificadores compuestos no existen** (`ActiveDraft`, `ActiveOrPending`, etc.) — el build falla con "Unknown qualifier". Para filtrar por "todos los status excepto X" usar `Non{X}`: ej. `countNonDiscontinuedProductsByCategoryId` genera `WHERE status <> 'DISCONTINUED' AND categoryId = :categoryId`.
...
- [ ] Cada código en `useCases[].fkValidations[].error` o `useCases[].fkValidations[].notFoundError` existe en `errors[].code`
